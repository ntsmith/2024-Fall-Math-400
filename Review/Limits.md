...
Let f be a function defined on an interval that contains $x=a$, except possibly at $x=a$.  Then
...
If $f(x) \leq g(x)$, then $\displaystyle \lim_{x\rightarrow a} f(x) \leq \lim_{x\rightarrow a} g(x)$.
